...
idle target is woken and a busy target receives ordinary queued work. Messages
are retained in memory for the lifetime of the plugin and carry sender, target,
...
- **In-memory retention** — messages are lost when the plugin process exits;
  durable inbox/outbox persistence is the next local-hardening step.
