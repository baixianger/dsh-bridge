...
class LocalSessionMessagingImpl {
	...
	async deliver({ id = crypto.randomUUID(), from, to, text, transport = "local" }) {
		...
		const delivery = this._deliver({ id, from, to, text, transport }).catch((error) => {
			this.deliveries.delete(key);
			throw error;
		});
		this.deliveries.set(key, delivery);
		while (this.deliveries.size > 10_000) this.deliveries.delete(this.deliveries.keys().next().value);
		return delivery;
	}
	async _deliver({ id, from, to, text, transport }) {
